...
export interface INavigationMap {
  addNewVs: (
    ...
    colId_xy: number[], // like ["-1,0"] ,["0,0"], ["0,1"],
    layerId: number
  ) => void;
  registerNavItem: (itemId: string) => void; // itemId should be structured through utilNavigation.generateItemId()
  updateMapData: (mapMeta: INavigationMapMeta) => void; // To force update activeState (i.e to focus an element forefully)
  navigate: (direction: ENavigationDirection) => INavigationMapMeta; // to trigger navigate, to be combined with arrow key events and respective direction
  getNextNavigate: (
    // This doesnt execute/commit the navigation, rather it returns the next focus state (if navigate() is triggered). This can be combined with updateMapData() to execute a navigation move
    direction: ENavigationDirection
  ) => INavigationMapMeta | undefined;
  getMapObj: () => INavigationMapObj; // Returns the complete map of items, rows, cols, and layer hierarchy
  getActiveState: () => INavigationMapState; // Returns active state or currently focused item, row, col and layer
  ready: () => void; // To be called when page is ready for navigation. This makes correction to activeState or initial focused Item, if needs correction.
}

export class NavigationMap implements INavigationMap {
  private map: INavigationMapObj = {
    activeLayer: 0,
    layers: {},
  };
  ...
  public addNewVs = (
    ...
    if (!this.map.layers[layerId]) {
      this.map.layers[layerId] = {
        lastFocusedVs: colId_xy,
        vss: {},
      };
    }

    const vsXYIdStr = utilNavigation.vsNumberArrToStr(colId_xy);
  ...
  public registerNavItem = (itemId: string) => {
  ...
  public getMapObj = (): INavigationMapObj => this.map;

  public getActiveState = (): INavigationMapState => this.activeState;
